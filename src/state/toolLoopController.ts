...
export const AGENT_LOOP_SYSTEM_PROMPT = `You are an autonomous coding agent in Orbit Code.
...
export function invalidRunCommandMessage(params: ToolParams): string | null {
...
export function normalizeApprovalOutcome(value: boolean | ToolApprovalOutcome, tool: string, params: ToolParams): ToolApprovalOutcome {
...
export class ToolLoopController {
  ...
  constructor(private readonly callbacks: ToolLoopCallbacks) {}
  ...
  async run(input: ToolLoopRunInput): Promise<ToolLoopResult> {
    ...
    return {
      summary,
      finalSummary: this.phase === "done" ? summary : undefined,
      stoppedForReview: this.phase === "reviewing",
    };
  }
      ...
      while (this.iteration < this.maxIterations && this.isRunning) {
        ...
        for (const tc of toolCalls) {
          ...
          try {
            ...
            } else if (this.callbacks.executeToolCall) {
              result = await this.callbacks.executeToolCall({ ...tc, name: toolName, status: "running" });
            } else {
