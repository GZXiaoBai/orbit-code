...
| Runtime permission contract | partially fixed | `PolicyEngine` and `PermissionScheduler` now decide Plan/Build tool allowance before UI approval. Plan-mode executable tools are denied without creating UI approvals, and Build-mode command approval uses the queue as an Adapter. Dynamic project-rule decisions cannot make the effective policy more permissive. `RuntimeLedger` and `ActionRequiredStore` now persist/replay blocking action state with run/tool metadata. Unit tests cover install/network ActionRequired classification, and E2E covers install denial without terminal execution. Localized install/network risk copy still needs refinement. |
| Review Dock stale queue pollution | fixed for current scope | Review Dock is now an Inspector projection for patch and terminal detail from scoped thread events; command/question actions are handled by central overlays. This reduces cross-thread/task confusion during resumed runs. |
...
| Workspace root | partially fixed | Agent runtime read/search/command calls now go through `WorkspaceGateway` and reject missing `workspacePath`; `search_code` uses the Rust `search_workspace_files` gateway. Some non-Agent Rust commands still use `std::env::current_dir()` compatibility fallback. |
...
- RuntimeLedger is now the selector input for pending actions/run steps, but Review Dock still accepts legacy approval/question adapters for compatibility; finish removing those from the main Inspector model before calling the ledger migration complete.
- Center approval overlays now render command/args/risk/reason, but risk copy still needs better localization and install/network-specific detail.
