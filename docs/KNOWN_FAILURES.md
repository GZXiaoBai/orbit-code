...
| E2E suite | fixed and verified | `npm run test:e2e` now passes 30/30. |
...
| Agent waiting-state recovery | fixed for pending UI | Session state now persists Agent run session, pending approvals, pending questions, patch proposals, terminal runs, and Agent events. Playwright reload tests cover command approval, question, and patch review recovery. Automatic model continuation after reload is intentionally not implemented. |
...
| Provider support mismatch | partially fixed | Ollama is exposed as model discovery only and is blocked from Build execution; chat/streaming relay is still not implemented. |
...
| API import memory after rename | fixed with migration path | Keychain storage now writes `orbit-code`, reads legacy `agent-gui` as fallback, and migrates legacy keys forward. SQLite now opens `orbit_code.db` and copies legacy `agent_gui.db` when present. Manual desktop smoke with the user's existing DeepSeek key is still recommended. |
...
| GitHub publication | blocked by tooling | The repo is GitHub-ready, but this environment has no `gh` CLI and the exposed GitHub plugin tools do not include repository creation/push. Local git initialization/commit can proceed, but remote upload still needs an authenticated push path. |
