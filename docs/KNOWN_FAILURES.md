...
| DeepSeek malformed tool output loops | partially fixed | The Agent prompt now requires exactly one one-line JSON tool call, rejects shell operators/`cd ... && ...`, strips fabricated tool-result text, and stops after repeated invalid/no-tool responses with an actionable recovery message instead of burning all iterations silently. Real DeepSeek smoke now has a repeatable runner; keep using it as a release gate. |
| Real DeepSeek smoke | verified for mini-lab runner paths | Manual desktop smoke was run on 2026-05-29 with DeepSeek `deepseek-v4-flash` against `/Users/zhoujunjie/PersonalProjects/test for orbit/orbit-mini-lab`. Passing path: Plan generated a 2-task draft for `SMOKE_LEDGER_20260529.md`, user accepted Build, patch sandbox preview passed, patch applied transactionally, command approval ran `npm run test:run`, verification passed, and final summary was produced. `npm run smoke:deepseek` then passed on 2026-05-30 for `happyPath`, `staleWriteRecovery`, and `contextPath`, including a simulated stale-write preview conflict/retry and explicit `ORBIT.md` / `.orbit/rules.md` / `.orbit/skills/smoke/SKILL.md` context evidence. |
...
- Real DeepSeek smoke has current passing mini-lab runner paths for happy path, stale-write recovery, and project rules/skills context. Keep it as a release gate and continue adding restored verification approval, restart resume, and desktop overlay UX cases.
