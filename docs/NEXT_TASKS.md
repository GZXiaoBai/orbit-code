...
2. Finish the credentialed `desktop-build-live` workflow. `smoke:live-vault:export` now writes a minimized encrypted vault DB that fits GitHub environment secret limits, `ORBIT_LIVE_VAULT_BUNDLE_B64` is set on `orbit-live-smoke`, and run `26933217124` proved `Bootstrap encrypted Orbit live vault` before being cancelled at the old full `tauri build --debug` stage. The workflow now builds the launchable debug binary with `prepare:codex-sidecar`, `npm run build`, and `cargo build --manifest-path src-tauri/Cargo.toml`; after committing/pushing that workflow update, rerun live CI and promote the live readiness plus approved/denied `npm run smoke:desktop-build` reports.
