...
| --- | --- | --- |
| `npm test -- --run` | `verified` | 98 Vitest tests passed, including file tree persistence, DeepSeek/provider capability fallback, keychain namespace migration helpers, Composer paste attachment classification, custom run controls, context compaction, recovered approval/question helpers, Review Dock queue model, provider smoke state, and run-session resume kind coverage. |
...
| Settings/API Key UI | `partial` | Independent settings workspace, real section list, Models import surface, DeepSeek capability fallback, provider smoke state, Security matrix, Appearance preferences, and Usage page are covered. | Fixture smoke is automated; real provider model discovery still needs manual/API-key smoke. |
| Provider persistence | `verified for migration helpers` | Frontend keychain storage now writes `orbit-code` and reads `orbit-code` then legacy `agent-gui`; SQLite DB path migrates legacy `agent_gui.db` into `orbit_code.db`; tests cover service order and DB copy. | Needs manual desktop smoke with an existing migrated Keychain entry. |
...
| Keychain | `partial` | `store_credential` / `get_credential` commands and frontend adapter exist; Orbit Code now reads legacy Agent GUI keys and migrates them forward. | Needs manual desktop validation on macOS/Windows/Linux. |
