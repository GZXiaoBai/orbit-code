...
| --- | --- | --- |
| `npm test -- --run` | `verified` | 155 Vitest tests passed, including typed ThreadEvent storage/projection, legacy AgentEvent migration, persisted approval grant scope, file tree persistence, DeepSeek/provider capability fallback, credential vault prefix coverage, Composer paste attachment classification, context compaction events, recovered approval/question helpers, Review Dock queue scoping, provider smoke state, thread isolation, and run-session resume/explicit-continue coverage. |
| `npm run build` | `verified` | TypeScript and Vite build passed on 2026-05-27; Vite still reports a Tauri API dynamic/static import chunk warning. |
| `cargo test --manifest-path src-tauri/Cargo.toml` | `verified` | 40 Rust tests passed, including sandbox patch preview, stale-write/path traversal coverage, provider host allowlist/model-list parsing, structured command cwd tests, credential vault encryption, Tauri dev workspace-root normalization, and SQLite legacy DB migration. |
| `npm run test:e2e` | `verified` | 34 Playwright tests passed, including real-workspace fixture, file tree + Monaco preview, Composer persistence, project permissions, multiple threads, thread archive/delete and clean new-thread isolation, settings layout, command approve/deny, ask_user question flow, reload recovery for pending command/question/patch review, Ollama discovery-only Build blocking, sandboxed patch review, transaction apply, conflict resolution, file preview refresh, and verification approval. |
| `npm run tauri build -- --debug` | `verified` | macOS `Orbit Code.app` and `Orbit Code_1.0.0_aarch64.dmg` debug bundles were produced successfully on 2026-05-27. |
...
| Conversation/timeline | `partial` | Timeline renders after Plan import and shows approval results. Thread menu, left-rail thread archive/delete, clean new-thread isolation, and Shift+Tab mode switching are covered. | Continue reducing hard-coded runtime messages. |
...
| Approval policy | `verified` | Agent `run_command` enters Review Dock approval, approve continues, deny returns a stop/replan signal and does not execute in fixture E2E. Approval requests now carry `once / session / project` grant scope, Review Dock command/verification cards expose it through the in-app `SelectMenu`, and session/project grants are persisted in session and thread snapshots. | Extend coverage to install/network-specific prompts and richer localized risk copy. |
...
| Agent Loop | `partial` | `agentLoopEngine.ts` has ReAct loop, streaming, strict JSON tool envelope parsing, command approval gate, first-class ask_user question queue, patch proposal handoff, and compaction. Agent run sessions are now scoped by `workspacePath + threadId + taskId + runSessionId`; continue execution resumes the current session instead of selecting a new task. Pending approvals, questions, patch proposals, terminal runs, and typed `ThreadEvent` records persist per session/thread snapshot; legacy `AgentEvent` is kept as a compatibility projection. Reload recovery is covered for pending command approval, question, and patch review, and user actions require an explicit “continue” before model execution resumes. Context compression now writes a first-class timeline event. | Full automatic model continuation after app restart is intentionally not implemented. Recovered actions perform explicit local resume work, write events, and then wait for the user to continue. Real DeepSeek smoke is still the main remaining acceptance path. |
...
| `src/components/thread/AgentTimeline.tsx` | `improved, still partial` | Timeline now consumes typed `ThreadEvent` projections for user messages, plans, command execution, patch proposals, questions, verification, and context compaction. It still owns sanitizer/display compaction and should keep shedding view-model logic. |
