...
On Linux CI, run packaged desktop WebDriver smokes inside both a DBus session and Xvfb, for example `dbus-run-session -- xvfb-run -a npm run smoke:desktop-plan`. The CI jobs only need a launchable `src-tauri/target/debug/orbit-code` binary, so they run `npm run tauri build -- --debug --no-bundle`: this keeps the Tauri CLI build environment and frontend asset embedding, but skips installer/app bundle packaging. The smoke reports include request/session/overall timeout values plus limited DOM diagnostics on failure; increase `ORBIT_WEBDRIVER_SESSION_TIMEOUT_MS` if a debug Tauri binary needs more time to create the first WRY session.
